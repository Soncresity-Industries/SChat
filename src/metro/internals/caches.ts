...
const SCHAT_METRO_CACHE_PATH = "caches/metro_modules.json";
...
export async function initMetroCache() {
    if (!await fileExists(SCHAT_METRO_CACHE_PATH)) return void buildInitCache();
    const rawCache = await readFile(SCHAT_METRO_CACHE_PATH);
