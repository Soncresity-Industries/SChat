...
import { updateSChatColor } from "./colors/updater";
...
export async function fetchTheme(url: string, selected = false) {
    ...
    if (selected) {
        ...
        updateSChatColor(themes[url].data, { update: true });
    }
}
...
export function selectTheme(theme: VdThemeInfo | null, write = true) {
    ...
    if (theme == null && write) {
        updateSChatColor(null, { update: true });
        return writeThemeToNative({});
    } else if (theme) {
        updateSChatColor(theme.data, { update: true });
