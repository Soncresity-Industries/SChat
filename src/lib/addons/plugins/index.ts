...
import { createSChatPluginApi } from "./api";
import * as t from "./types";

type PluginInstantiator = (
    schat: t.SChatPluginObject,
...
export const registeredPlugins = new Map<string, t.SChatPluginManifest>();
export const pluginInstances = new Map<string, t.PluginInstanceInternal>();
export const apiObjects = new Map<string, ReturnType<typeof createSChatPluginApi>>();
...
function isExternalPlugin(manifest: t.SChatPluginManifest): manifest is t.SChatPluginManifestInternal {
...
export async function updateAndWritePlugin(repoUrl: string, id: string, fetchScript: boolean) {
    const manifest: t.SChatPluginManifestInternal = await fetchJSON(repoUrl, `builds/${id}/manifest.json`);
...
export async function updateRepository(repoUrl: string) {
    ...
    for (const id of pluginIds) {
        const manifest = getPreloadedStorage<t.SChatPluginManifest>(`plugins/manifests/${id}.json`);
    ...
    if (isExternalPlugin(manifest)) {
        ...
        try {
            ...
            var instantiator = globalEvalWithSourceUrl(
                `(schat,definePlugin)=>{${iife};return plugin?.default ?? plugin;}`,
                `schat-plugin/${id}-${manifest.version}`
        ...
        try {
            const api = createSChatPluginApi(id);
